    ...
    <div class="card-body">
        <form action="{{ route('admin.supports.store') }}" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="number" class="form-control @error('whatsapp') is-invalid @enderror" name="whatsapp" value="{{ old('whatsapp') }}" required>
                @error('whatsapp')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            <div class="row g-3">
                <div class="col-12 col-md-2 form-group">
                    ...
                    <select class="form-control @error('status') is-invalid @enderror" name="status">
                        ...
                        <option {{ old('status') == 0 ? 'selected' : '' }} value="0">Off</option>
                    </select>
                    @error('status')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
