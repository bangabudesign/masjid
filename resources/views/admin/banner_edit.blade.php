    ...
    <div class="card-body">
        <form action="{{ route('admin.banners.update',  ['id' => $banner->id]) }}" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <div x-data="imageViewer('{{ $banner->image_url }}')">
                    <div class="mb-2">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control @error('link') is-invalid @enderror" name="link" value="{{ old('link', $banner->link) }}">
                @error('link')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            <div class="row g-3">
                <div class="col-12 col-md-2 form-group">
                    ...
                    <select class="form-control @error('status') is-invalid @enderror" name="status">
                        <option {{ old('status', $banner->status) == 1 ? 'selected' : '' }} value="1">Aktif</option>
                        <option {{ old('status', $banner->status) == 0 ? 'selected' : '' }} value="0">Off</option>
                    </select>
                    @error('status')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
