    ...
    <div class="card-body">
        <form action="{{ route('admin.products.update', ['id' => $product->id]) }}" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <div x-data="imageViewer('{{ $product->image_url }}')">
                    <div class="mb-2">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control @error('slug') is-invalid @enderror" name="slug" value="{{ old('slug', $product->slug) }}">
                @error('slug')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            <div class="form-group">
                ...
                <textarea class="form-control @error('body') is-invalid @enderror" name="body" id="editor" rows="10">{{ old('body', $product->body) }}</textarea>
                @error('body')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            <div class="row g-3">
                ...
                <div class="col-12 col-md-2 form-group">
                    ...
                    <select class="form-control @error('is_active') is-invalid @enderror" name="is_active">
                        <option {{ old('is_active', $product->is_active) == 1 ? 'selected' : '' }} value="1">Aktif</option>
                        <option {{ old('is_active', $product->is_active) == 0 ? 'selected' : '' }} value="0">Tidak Aktif</option>
                    </select>
                    @error('is_active')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
