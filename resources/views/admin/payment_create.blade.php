...
<div class="card border-0">
    <div class="card-body">
        <form action="{{ route('admin.payments.store', ['paymentable_type' => $paymentable_type, 'paymentable_id' => $paymentable_id]) }}" method="post" enctype="multipart/form-data">
            @csrf
            ...
            <div class="row g-3">
                <div class="col-12 col-md-4 form-group">
                    ...
                    <select class="form-control @error('status') is-invalid @enderror" name="status">
                        <option {{ old('status') == 0 ? 'selected' : '' }} value="0">Unpaid</option>
                        <option {{ old('status') == 1 ? 'selected' : '' }} value="1">Paid</option>
                        <option {{ old('status') == -1 ? 'selected' : '' }} value="-1">Cancel</option>
                    </select>
                    @error('status')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="col-12 col-md-4 form-group">
                    ...
                    <input type="datetime-local" class="form-control @error('confirm_at') is-invalid @enderror" name="confirm_at" value="{{ old('confirm_at') }}">
                    @error('confirm_at')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
